    ...
    <main className="min-h-screen bg-gradient-to-b from-white via-primary-50/30 to-primary-50/5 pb-24 pt-32 dark:from-dark-gradient-from dark:via-dark-gradient-via/40 dark:to-dark-gradient-to">
      <div className="mx-auto flex flex-col gap-12 px-4 sm:px-6 lg:grid lg:grid-cols-[minmax(0,1fr)_340px] lg:items-start lg:gap-16 lg:px-8">
        <article className="w-full" data-blog-article>
          ...
          <h1 className="mt-6 text-4xl font-bold text-gray-900 dark:text-white">{post.title}</h1>
          ...
          <div className="prose prose-lg mt-10 max-w-none text-gray-700 prose-headings:text-gray-900 prose-strong:text-gray-900 prose-em:text-gray-900 prose-li:text-gray-700 prose-a:text-primary-600 dark:text-gray-100 dark:prose-headings:text-white dark:prose-strong:text-white dark:prose-em:text-gray-100 dark:prose-li:text-gray-100 dark:prose-a:text-primary-300">
            <MDXContent />
          </div>
        </article>

        <aside className="w-full space-y-8 rounded-3xl border border-white/40 bg-white/70 p-6 shadow-xl backdrop-blur dark:border-white/10 dark:bg-dark-card/70 lg:hidden">
          <div>
            <h2 className="text-base font-semibold text-gray-900 dark:text-white">Related reading</h2>
            <div className="mt-4 space-y-4">
              {relatedPosts.map((related) => (
                <Link key={related.slug} href={`/blog/${related.slug}`} className="block rounded-2xl border border-gray-100 px-4 py-3 text-sm text-gray-700 transition hover:border-primary-200 hover:text-primary-600 dark:border-white/10 dark:text-gray-300 dark:hover:border-primary-500/40">
      ...
      <section className="container mx-auto mt-16 px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">Keep exploring</h2>
