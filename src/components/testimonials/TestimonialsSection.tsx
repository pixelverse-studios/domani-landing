...
export function TestimonialsSection({
  ...
  const rootBackground =
    background === 'gradient'
      ? 'bg-gradient-to-b from-white via-primary-50/30 to-primary-50/5 dark:from-dark-gradient-from dark:via-dark-gradient-via/40 dark:to-dark-gradient-to'
      : 'bg-transparent'
    ...
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="mt-4 text-3xl font-bold text-gray-900 dark:text-white sm:text-4xl">{heading}</h2>
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {testimonials.map((testimonial) => (
            <figure
              ...
              className="flex h-full flex-col justify-between rounded-3xl border border-white/60 bg-white/90 p-6 text-left shadow-lg shadow-primary-500/10 backdrop-blur dark:border-white/10 dark:bg-dark-card/80"
            >
              <div>
                <div className="flex items-center gap-2 text-sm font-semibold text-amber-500">
                  {Array.from({ length: 5 }).map((_, index) => (
                    <svg
                      key={index}
                      className={`h-4 w-4 ${index + 1 <= Math.round(testimonial.rating) ? 'text-amber-500' : 'text-gray-300 dark:text-gray-600'}`}
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      aria-hidden="true"
                    >
                  ...
                  <span className="text-xs text-gray-500 dark:text-gray-400">{testimonial.rating.toFixed(1)}/5</span>
                </div>
                <blockquote className="mt-4 text-base text-gray-700 dark:text-gray-200">{testimonial.quote}</blockquote>
              </div>
