...
export function HeroMotionLayer() {
  ...
  return (
    <>
      <motion.div
        className="absolute inset-0 -z-10 bg-gradient-to-b from-white/0 via-white/10 to-white/0"
        initial={{ opacity: 0 }}
        animate={{ opacity: [0, 0.35, 0.2] }}
        transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="pointer-events-none absolute top-1/2 left-1/2 -z-10 h-[640px] w-[640px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-gradient-radial from-primary-400/20 to-transparent"
