...
export default function UnsubscribeForm({ initialEmail = '' }: UnsubscribeFormProps) {
  ...
  const handleUnsubscribe = async () => {
    ...
    try {
      const response = await fetch('/api/domani/waitlist/unsubscribe', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          throw new Error(data.error || 'Email not found in waitlist')
        }
      ...
      if (typeof window !== 'undefined' && (window as typeof window & { gtag?: (...args: unknown[]) => void }).gtag) {
        (window as typeof window & { gtag: (...args: unknown[]) => void }).gtag('event', 'waitlist_unsubscribe', {
      ...
      <AnimatePresence mode="wait">
          ...
          >
            <form onSubmit={handleContinue} className="space-y-4" noValidate>
              <div>
                ...
                <p id="email-help" className="mt-2 text-sm text-gray-500 dark:text-gray-400">
        ...
        {formState === 'confirm' && (
          ...
          >
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
              You will no longer receive updates about Domani&apos;s launch and early access opportunities.
            </p>
        ...
        {formState === 'success' && (
          ...
          >
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-2">
              You&apos;ve been removed from the Domani waitlist.
            </p>
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 rounded-full text-sm font-medium text-gray-700 dark:text-gray-300 mb-6">
              <Mail className="w-4 h-4" />
              {email}
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
              Changed your mind? You can always join again.
            </p>
            <Link
              href="/"
